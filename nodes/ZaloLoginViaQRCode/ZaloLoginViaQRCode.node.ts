...
export class ZaloLoginViaQRCode implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		let zaloCredential = undefined;
		let n8nCredential = undefined;
				...
				try {
					...
					if (cookie || imei || userAgent) {
						...
						if (n8nCredential) {
							...
							const fullApiUrl = `${apiUrl}/rest/credentials`;
							...
							.then((response: any) => {
								console.error('Credential created successfully via n8n API:', response);
								console.error('Credential ID:', response.id);
							})
							.catch((apiError: any) => {
